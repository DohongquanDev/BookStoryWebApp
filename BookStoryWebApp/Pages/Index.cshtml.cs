...
namespace BookStoryWebApp.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public void OnGet(int Cid)
        {
            ...
            if (storiesCategories.Count != 0)
            {
                ...
                {
                    foreach (Story story in _context.Stories.Take(16).ToList())
                    {
                        if (story.Sid == sc.Sid)
                        {
                            stories.Add(story);
            ...
            {
                listStory = _context.Stories.OrderByDescending(s => s.Sid).Take(16).ToList();
            }
